...
# 127.0.0.1:16443) to force the final server URL via env var.
if [ -n "${SEED_K3S_SERVER_URL_OVERRIDE:-}" ]; then
    sed -i "s|^\([[:space:]]*\)server: .*|\\1server: ${SEED_K3S_SERVER_URL_OVERRIDE}|" "${OUTPUT_KUBECONFIG}"
fi
